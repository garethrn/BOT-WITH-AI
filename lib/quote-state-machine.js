...
function getRequestValue(requestDetails, ...keys) {
    for (const key of keys) {
        const value = requestDetails?.[key];
...
function parseRequestDimensions(requestDetails = {}) {
    const dimensions = requestDetails?.dimensions;
    if (!dimensions || typeof dimensions !== 'object') return null;
    const unit = String(dimensions.unit || 'mm').toLowerCase();
    const convert = (value) => {
    ...
    const widthMm = convert(dimensions.widthMm ?? dimensions.width);
    const heightMm = convert(dimensions.heightMm ?? dimensions.height);
    if (!widthMm || !heightMm) return null;
    return { widthMm, heightMm };
}
...
function getRowFamilyLabel(row = {}) {
    return String(row.Name || row.Subcategory || row.Category || '').trim();
}

function getRowFamilyKey(row = {}) {
    return normalizeValue(getRowFamilyLabel(row));
}

function lockFamilyFromRows(state, rows = []) {
    const first = Array.isArray(rows) ? rows[0] : null;
    if (!first) return;
    state.lockedFamilyKey = getRowFamilyKey(first);
    state.lockedFamilyLabel = getRowFamilyLabel(first);
}

function filterRowsByLockedFamily(rows = [], lockedFamilyKey = '') {
    if (!lockedFamilyKey) return Array.isArray(rows) ? rows : [];
    return (Array.isArray(rows) ? rows : []).filter((row) => getRowFamilyKey(row) === lockedFamilyKey);
}
...
function isSwitchProductIntent(text = '') {
    const normalized = normalizeValue(text);
    if (!normalized) return false;
    if (/\b(switch|change|different|another|instead|new)\b/.test(normalized) && /\b(product|quote|item|job|service|design|card|banner|sign|sticker|flyer|label)\b/.test(normalized)) {
        return true;
    }
    if (/\b(switch to|change to|instead of)\b/.test(normalized)) return true;
    return false;
}
...
function handleQuoteConversationMessage({ jid, text, products, stateStore, requestDetails = null }) {
    ...
    if (hasSession && isSwitchProductIntent(input)) {
        const nextState = createState();
        nextState.productQuery = extractProductQuery(input);
    ...
    if (typeof requestDetails?.requested_poles === 'boolean') state.selection.polesRequested = requestDetails.requested_poles;
    if (typeof requestDetails?.requestedPoles === 'boolean') state.selection.polesRequested = requestDetails.requestedPoles;
    ...
    if (typeof requestDetails?.requested_installation === 'boolean') state.selection.installationRequested = requestDetails.requested_installation;
    if (typeof requestDetails?.requestedInstallation === 'boolean') state.selection.installationRequested = requestDetails.requestedInstallation;
    ...
    while (guard < 10) {
        ...
        if (state.step === STATES.AWAITING_PRODUCT) {
            ...
            lockFamilyFromRows(state, state.candidates);
            state.candidates = filterRowsByLockedFamily(state.candidates, state.lockedFamilyKey);
        ...
        if (state.step === STATES.AWAITING_SIZE) {
            ...
            if (sizeOptions.length > 1 && !state.selection.size) {
                const requestSize = String(getRequestValue(requestDetails, 'size') || '').trim();
                const picked = (requestSize && findMatchingOption(requestSize, sizeOptions)) || findMatchingOption(input, sizeOptions);
                if (picked) {
                    state.selection.size = picked.display;
                    const filtered = filterRowsByLockedFamily(filterRows(state.candidates, { size: picked.display }), state.lockedFamilyKey);
                    if (!filtered.length) {
                        stateStore.set(jid, state);
        ...
        if (state.step === STATES.AWAITING_FINISH) {
            ...
            if (finishOptions.length > 1 && !state.selection.finish) {
                const requestFinish = String(getRequestValue(requestDetails, 'finish') || '').trim();
                const picked = (requestFinish && findMatchingOption(requestFinish, finishOptions)) || findMatchingOption(input, finishOptions);
                if (picked) {
                    state.selection.finish = picked.display;
                    const filtered = filterRowsByLockedFamily(filterRows(state.candidates, { finish: picked.display }), state.lockedFamilyKey);
                    if (!filtered.length) {
                        stateStore.set(jid, state);
        ...
        if (state.step === STATES.AWAITING_SIDED_OPTION) {
            ...
            if (sideOptions.length > 1 && !state.selection.sided) {
                const requestSided = String(getRequestValue(requestDetails, 'sided_option', 'sidedOption') || '').trim();
                const parsedSide = normalizeSidedOption(requestSided || input);
                const picked = sideOptions.find((item) => item.value === parsedSide) || findMatchingOption(requestSided || input, sideOptions);
                if (picked) {
                    state.selection.sided = picked.display;
                    const filtered = filterRowsByLockedFamily(filterRows(state.candidates, { sided: picked.display }), state.lockedFamilyKey);
                    if (!filtered.length) {
                        stateStore.set(jid, state);
        ...
        if (state.step === STATES.AWAITING_ARTWORK_STATUS) {
            if (state.selection.hasArtwork === null) {
                const requestArtwork = getRequestValue(requestDetails, 'has_artwork', 'hasArtwork');
                const parsed = typeof requestArtwork === 'boolean' ? requestArtwork : parseArtworkStatus(input);
        ...
        if (state.step === STATES.AWAITING_DIMENSIONS) {
            if (!state.selection.dimensions) {
                const parsed = parseRequestDimensions(requestDetails) || parseDimensionsFromText(input);
                if (!parsed) {
                    const count = recordAsk(state, state.step);
                    if (count > loopLimit) {
                        stateStore.set(jid, createState());
        ...
        if (state.step === STATES.AWAITING_QUANTITY) {
            if (!state.selection.quantity) {
                const extractedQty = Number(getRequestValue(requestDetails, 'quantity'));
                const qty = Number.isFinite(extractedQty) && extractedQty > 0 ? extractedQty : parseQuantityFromText(input);
                if (!qty) {
                    const count = recordAsk(state, state.step);
                    if (count > loopLimit) {
                        stateStore.set(jid, createState());
                    ...
                    const qtyOptions = [...new Set(state.candidates
                        .filter((row) => normalizeValue(row.PriceType) === 'fixed')
                        .map((row) => parseUnitsPerProduct(row.UnitsPerProduct))
                        .filter(Boolean))];
                    const qtyPrompt = qtyOptions.length
                        ? `What quantity should I quote for ${state.lockedFamilyLabel || 'this product'}? Available quantity options: ${qtyOptions.slice(0, 5).join(', ')}.`
        ...
        if (state.step === STATES.QUOTE_READY) {
            ...
            const candidates = filterRowsByLockedFamily(selectedRows.length ? selectedRows : state.candidates, state.lockedFamilyKey);
            ...
            return {
                handled: true,
                reply: formatQuoteBreakdown(result, state.selection),
                diagnostic: {
                    lockedFamily: state.lockedFamilyLabel || '',
                    step: state.step,
