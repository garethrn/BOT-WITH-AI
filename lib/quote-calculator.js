...
function parseFixedPricingRule(product = {}) {
    ...
    const isUnitMode = /^(?:>=|>)\s*\d+/.test(rawUnits) || modeHint === 'unit' || numericUnitPrice;
    const minQty = isUnitMode ? (minOrderQty > 0 ? minOrderQty : units) : 0;
    return { mode: isUnitMode ? 'unit' : 'pack', unitsPerPack: units, minQty };
}

function resolveFixedPrice(product = {}) {
    const fixedPrice = toNumber(product.FixedPrice);
    if (fixedPrice > 0) return fixedPrice;
    const unitPriceFallback = toNumber(product.UnitPricing);
    if (unitPriceFallback > 0) return unitPriceFallback;
    return 0;
}
...
function calculateQuote(product, input = {}) {
    ...
    if (priceType === 'sqm') {
        ...
        const fixedPrice = resolveFixedPrice(product);
        if (!(fixedPrice > 0)) {
            return { ok: false, code: 'empty_price_fields', message: 'This product is missing fixed pricing in the CSV (FixedPrice/UnitPricing).' };
        }
