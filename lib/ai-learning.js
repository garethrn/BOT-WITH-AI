...
function normalizeWhenKey(value) {
    const normalizedRule = normalizeRuleText(value);
...
function importBackup(fileName, fileContent, options = {}) {
    ...
    for (const rule of rules) upsertResponseRule(data, rule);
...
function upsertResponseRule(data, pair) {
    if (!data || !pair || !pair.when || !pair.reply) return false;
    const when = normalizeWhenKey(pair.when);
    const reply = normalizeMessageText(pair.reply);
    if (!when || !reply) return false;
...
function learnFromChatMessages(messages = [], options = {}) {
    const list = Array.isArray(messages) ? messages : [];
    if (list.length < 2) return { importedRules: 0, inspectedMessages: list.length, updates: 0 };
    const data = readData();
    const maxPairs = Math.max(1, Number(options.maxPairs || 80));
    let importedRules = 0;
    let updates = 0;
    ...
    for (let i = 1; i < list.length; i += 1) {
        ...
        if (!previous || !current) continue;
        if (String(previous.role || '').toLowerCase() !== 'user') continue;
        const currentRole = String(current.role || '').toLowerCase();
        if (currentRole !== 'bot' && currentRole !== 'assistant' && currentRole !== 'admin') continue;
        const when = normalizeWhenKey(previous.text);
        const reply = normalizeMessageText(current.text);
        if (!isUsableMessage(when) || !isUsableMessage(reply)) continue;
        if (tokenizeRuleText(when).length < 2) continue;

        const existingBefore = data.responseRules.find((rule) => rule.when === when);
        const existingReplyBefore = existingBefore ? String(existingBefore.reply || '') : '';
        const changed = upsertResponseRule(data, { when, reply });
        if (!changed) continue;
        importedRules += 1;
        if (existingBefore && existingReplyBefore !== reply) updates += 1;
        if (importedRules >= maxPairs) break;
    }

    saveData(data);
    logAIActivity('chat_learning_saved', {
        importedRules,
        updates,
        inspectedMessages: list.length,
        source: String(options.source || 'selected_chat')
    });
    return { importedRules, inspectedMessages: list.length, updates };
}
...
function buildOpenAISystemPrompt() {
    ...
    const sections = [
        ...
        [
            ...
            '1) Talk naturally like a helpful human sales assistant.',
            '2) Ask one question at a time to gather missing details.',
            '3) For products and prices, rely only on the Products CSV context. Never guess products or pricing.',
            '4) Learn from useful conversations and dashboard teachings to improve helpful replies over time.'
        ].join('\n')
    ];
    ...
    sections.push('If no specific pattern applies, provide a helpful general response with one clear next question.');
    return sections.join('\n\n');
}

function rememberConversationReply(userText, assistantReply) {
    const when = normalizeWhenKey(userText);
...
function applyImmediateCorrection(userQuestion, correctedReply, options = {}) {
    const when = normalizeWhenKey(userQuestion);
    const reply = normalizeMessageText(correctedReply);
    if (!isUsableMessage(when) || !isUsableMessage(reply)) {
        return { applied: false, reason: 'invalid_input', rules: 0, removedMemories: 0 };
    }
    const data = readData();
    const changed = upsertResponseRule(data, { when, reply });
    const before = data.conversationReplies.length;
    data.conversationReplies = data.conversationReplies.filter((item) => normalizeWhenKey(item.when) !== when);
    const removedMemories = Math.max(0, before - data.conversationReplies.length);
    saveData(data);
    logAIActivity('correction_saved', {
        source: String(options.source || 'new_ai_learning'),
        whenLength: when.length,
        replyLength: reply.length,
        rulesTotal: data.responseRules.length,
        removedMemories
    });
    return {
        applied: changed || removedMemories > 0,
        reason: changed ? 'rule_saved' : 'already_up_to_date',
        rules: data.responseRules.length,
        removedMemories
    };
}
